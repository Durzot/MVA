...
class FullGMM(object):
    ...
    def fit(self, X):
        ...
        self.Sigma2 = np.array([np.identity(d)]*self.k)
    ...
    def _get_tau(self, X, pi, mu, Sigma2):
        ...
        for i in range(n):
            for j in range(self.k):
                tau[i,j] = pi[j]*np.linalg.det(Sigma2[j])**(-1/2.)*np.exp(- 1./2.*(X[i]-mu[j]).dot(np.linalg.inv(Sigma2[j])).dot(X[i]-mu[j]))
    ...
    def _update_Sigma2(self, X, mu, tau):
        n, d = X.shape
        Sigma2 = np.array([np.identity(d)]*self.k)
        for j in range(self.k):
            for i in range(n):
                Sigma2[j] += (X[i]-mu[j]).reshape(-1, 1).dot((X[i]-mu[j]).reshape(1, -1))*tau[i, j]
            Sigma2[j] /= tau[:, j].sum()
        return Sigma2
    ...
    def _get_lc(self, X, pi, mu, Sigma2, labels):
        ...
        for i in range(n):
            for j in range(k):
                if labels[i] == j:
                    lc += np.log(pi[j]) - d/2.*np.log(2*np.pi) - 1./2.*np.log(np.linalg.det(Sigma2[j])) - 1./2.*(X[i]-mu[j]).dot(np.linalg.inv(Sigma2[j])).dot(X[i]-mu[j])
        return lc
